...
// Twitter banner: 1500x500 (3:1 ratio)
...
export default function BannerPage() {
  ...
  const drawCanvas = useCallback(() => {
    ...
    drawCoverImage(ctx, bg1, CANVAS_WIDTH, CANVAS_HEIGHT);

    // Draw background 2
    ctx.globalAlpha = settings.blend / 100;
    drawCoverImage(ctx, bg2, CANVAS_WIDTH, CANVAS_HEIGHT);

    // Draw logos
    ctx.globalAlpha = settings.logoOpacity / 100;
    ctx.globalCompositeOperation = settings.logoBlendMode;

    // Logo sized relative to canvas height for banner
    const logoWidth = CANVAS_HEIGHT * 1.8;
    ...
    for (let i = 0; i < totalLogos; i++) {
      ...
      ctx.save();
      ctx.translate(CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2 + yOffset);
    ...
    const sloganFontSize = 24 * (settings.sloganScale / 100);
    ctx.font = `600 ${sloganFontSize}px ui-monospace, SFMono-Regular, "SF Mono", Menlo, monospace`;
    ctx.fillStyle = settings.logoColor;
  ...
  const handleDownload = () => {
    ...
    link.download = `endg4me-banner-${Date.now()}.png`;
  ...
  return (
    <main className="flex min-h-screen flex-col bg-zinc-950 p-4 lg:flex-row">
      {/* Preview */}
      <div className="flex flex-1 items-center justify-center p-4">
        <canvas
          ...
          className="h-auto max-h-[50vh] w-auto max-w-full rounded-sm lg:max-h-[60vh]"
        />
      </div>

      {/* Controls */}
      <div className="flex w-full flex-col gap-4 overflow-y-auto rounded-lg border border-zinc-800 bg-zinc-900 p-4 lg:w-80">
        <h2 className="font-mono text-sm font-semibold text-zinc-300">
          Banner Controls (1500x500)
        </h2>
        ...
        {/* Slogan Scale */}
        <Control label="Slogan Scale" value={`${settings.sloganScale}%`}>
          <input
            type="range"
            min="0"
            max="300"
            value={settings.sloganScale}
            onChange={(e) => updateSetting("sloganScale", Number(e.target.value))}
            className="slider"
          />
        </Control>

        {/* Slogan Offset Y */}
        <Control label="Slogan Offset Y" value={`${settings.sloganOffsetY}px`}>
          <input
            type="range"
            min="-200"
            max="200"
            value={settings.sloganOffsetY}
            onChange={(e) => updateSetting("sloganOffsetY", Number(e.target.value))}
            className="slider"
          />
        </Control>
